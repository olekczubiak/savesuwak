...
def save_to_file(data: str, filename: str = MY_TIME[:10]):
...
def return_all_data(json_data):
    for n in range(number_of_elements(json_data)):
        title = str(parse_data(json_data, n, 'title').decode()).replace('"', '')
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if n == 3:
            ...
            html = f"<html><head></head><body><p>Audycja z dzisiaj<br> <ol> {output.encode('ascii', 'ignore')[0:3] + output.encode('ascii', 'ignore')[6:]} </ol></body></html>"
            # print(html)
            gm = Gmail(SENDER_MAIL, PASSWORD)
            gm.send_message('TEST - 4', RECIVER_MAIL, html)
            break
